...
**1. Factual Accuracy** - Is the information in the output actually true? This includes not just obvious facts but subtle claims, statistics, dates, attributions, and technical details. The [hallucination problem](/learn/what-is-ai-hallucination) lives here.
...
One failure mode I didn't expect to find as often as I did: format pollution from retrieval. Tools that use [RAG - retrieval-augmented generation](/learn/what-is-rag) sometimes pull in source text with inconsistent formatting and pass that noise into the output. The output looks sloppy not because the model is bad at formatting but because the retrieved chunks are. If you're evaluating a RAG-based tool, probe this specifically by asking questions where the answer requires synthesizing multiple retrieved chunks.
...
For teams evaluating [AI agents](/learn/what-is-an-ai-agent) rather than single-turn models, the automation calculus shifts. Agent evaluation often requires checking intermediate steps, not just final outputs. Automating intermediate-step evaluation is hard and expensive, which means human evaluation stays more central for agent workflows even at scale.

If you're deciding whether to invest in [cloud AI vs local AI](/learn/cloud-ai-vs-local-ai) for evaluation infrastructure, note that running LLM-as-judge evaluations locally is now practical for many teams. Local models like Llama 3.1 70B have decent inter-rater reliability for instruction-following and format checks, though they still lag on subtle quality distinctions.
...
The real-world cross-check is the part I'm most proud of in this methodology. Evaluation in a lab tells you how a model performs on your specific prompts. It doesn't tell you how it performs across thousands of different user prompts with different levels of [prompt engineering skill](/learn/what-is-prompt-engineering). We collect user feedback, monitor community forums, and occasionally run user surveys as a check on our lab scores.
...
The cleanest approach is to score quality first and separately, then apply a cost-effectiveness adjustment. Calculate cost per 1,000 quality-scored outputs at each model's pricing tier. The question shifts from "which model is best?" to "which model delivers the most quality per dollar at my usage volume?" Our [AI ROI calculator guide](/learn/ai-tools-roi) walks through this math in more detail for teams running these comparisons at scale.
