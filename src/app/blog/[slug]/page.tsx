...
import { getBlogBySlug, getAllBlogSlugs, getRelatedContent } from "@/lib/content";
...
import { isBlogIndexable, NOINDEX_ROBOTS } from "@/lib/indexing";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    ...(!isBlogIndexable(slug) && { robots: NOINDEX_ROBOTS }),
...
export default async function BlogPostPage({ params }: Props) {
  ...
  const ogUrl = `https://rawpickai.com/api/og?title=${encodeURIComponent(fm.title)}&type=Blog&category=${encodeURIComponent(fm.category || "")}`;
  const canonical = `https://rawpickai.com/blog/${slug}`;
    ...
    "@graph": [
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 2, name: "Blog", item: "https://rawpickai.com/blog" },
