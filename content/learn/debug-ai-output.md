...
**Too many tasks in one prompt.** Asking the model to analyze, summarize, reformat, and recommend all at once. The signature: some subtasks are handled well, others are abbreviated or skipped. Fix: break it into sequential prompts, especially for [complex document analysis tasks](/learn/structure-documents-for-ai).
...
If you want a systematic approach to building better prompts from the start - rather than fixing them after the fact - the [prompt engineering guide](/learn/what-is-prompt-engineering) covers the foundational principles, and [10 prompt patterns that always work](/learn/prompt-patterns) gives you reusable templates. Those two together eliminate the most common prompt errors before they happen.
...
For use cases where you regularly need to provide large amounts of domain context - internal documents, company policies, specialized knowledge bases - the [guide to training AI on your own data](/learn/train-ai-on-your-data) covers the more permanent solutions. For one-off tasks, structured document preparation as covered in [how to structure documents for AI analysis](/learn/structure-documents-for-ai) gives you a repeatable process that doesn't require any infrastructure investment.
...
The log also serves as a personal knowledge base for [prompt engineering](/learn/write-better-ai-prompts). When you encounter a task type you've debugged before, you can look up what worked. You're building a personal dataset of what works for your specific use cases with your specific writing patterns - something no generic prompt guide can replicate.
...
The debug log also connects directly to how you [evaluate AI output quality](/learn/evaluate-ai-output) over time. If you're tracking both what went wrong and what quality looks like when things go right, you're building a personal evaluation framework specific to your workflows.
