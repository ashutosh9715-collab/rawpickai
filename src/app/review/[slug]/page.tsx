...
export default async function ReviewPage({ params }: Props) {
  ...
  const pricingMatch = review.content.match(/\$(\d+)\/mo/);
  ...
  const ftLabel = ftNorm >= 4 ? "Excellent" : ftNorm >= 3 ? "Good" : ftNorm >= 2 ? "Limited" : fts ? "Poor" : "N/A";
  const ftColor = ftLabel === "Excellent" || ftLabel === "Good" ? "#22c55e" : ftLabel === "Limited" ? "#f59e0b" : "#ef4444";

  const verdictMatch = review.content.match(/(?:verdict|bottom line|final)[:\s]*(.{50,200}?)(?:\.|$)/i);
  const verdict = verdictMatch ? verdictMatch[1].trim() + "." : fm.description || "";
  ...
  const faqSchema = {
    ...
    mainEntity: [
      ...
      },
      {
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text: `${toolName} is ${fm.description || `an AI tool reviewed at RawPickAI`}. See our full review for use cases, features, and who it is best suited for.`,
        },
      },
    ],
  };
  ...
  return (
    <>
      ...
      <div className="max-w-[760px] mx-auto px-5 md:px-10 py-8 md:py-10 pb-16 md:pb-20">
        ...
        <div className="prose-content" dangerouslySetInnerHTML={{ __html: review.htmlContent }} />
